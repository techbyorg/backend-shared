...
async function getSchema ({ directives = [], typeDefs = [], dirName }) {
  ...
  let resolversArray = _.filter(await Promise.map(graphqlFolders, async (folder) => {
    return import(path.join(dirName, `./graphql/${folder}/resolvers.js`))
      .then((module) => module.default)
      .catch((err) => {
        if (err.code !== 'ERR_MODULE_NOT_FOUND' || err.message.indexOf("resolvers.js' imported") === -1) {
          console.error('error loading', folder, err.message)
        }
        return null
      })
  }))
