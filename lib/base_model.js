...
export default class Base {
  ...
  async search ({ query, sort, limit, trackTotalHits, isRandomized }) {
  ...
  // requires this.getBySlug to exist
  getUniqueSlug (baseSlug, suffix, attempts = 0) {
    ...
    return this.getBySlug(slug)
      .then(existingPlace => {
        ...
        if (existingPlace?.id) {
          return this.getUniqueSlug(baseSlug, (suffix || 0) + 1, attempts + 1)
        } else {
          return slug
        }
      })
  ...
  defaultOutput (row) {
    ...
    return _.mapValues(row, (value, key) => {
      const { type, subType, defaultFn, defaultOutputFn } = this.fieldsWithType[key] || {}
