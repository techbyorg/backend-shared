...
const contactPoints = config.get().SCYLLA.CONTACT_POINTS;

const cassanknexInstance = cassanknex({
  connection: {
    contactPoints,
    pooling: {
...
const ready = new Promise((resolve, reject) => cassanknexInstance.on('ready', function(err, res) {
...
cknex.setDefaultKeyspace = keyspace => defaultKeyspace = keyspace;

cknex.getTimeUuid = function(time) {
  if (time) {
